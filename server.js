...
CREATE TABLE IF NOT EXISTS review_submissions(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,order_id INTEGER NOT NULL UNIQUE,gameplay_url TEXT NOT NULL,notes TEXT, status TEXT NOT NULL DEFAULT 'submitted',created_at TEXT NOT NULL,FOREIGN KEY(user_id) REFERENCES users(id),FOREIGN KEY(order_id) REFERENCES orders(id));
`);
...
const services={
 ...
 champs1:{name:"FUT Champions Rank 1 Coaching",amount:5500,description:"FUT Champions coaching focused on a Rank 1 target. You keep full control of your account.",mode:"payment"},
 champs2:{name:"FUT Champions Rank 2 Coaching",amount:5000,description:"FUT Champions coaching focused on a Rank 2 target. You keep full control of your account.",mode:"payment"},
 champs3:{name:"FUT Champions Rank 3 Coaching",amount:4000,description:"FUT Champions coaching focused on a Rank 3 target. You keep full control of your account.",mode:"payment"},
 champs4:{name:"FUT Champions Rank 4 Coaching",amount:3000,description:"FUT Champions coaching focused on a Rank 4 target. You keep full control of your account.",mode:"payment"},
 champs5:{name:"FUT Champions Rank 5 Coaching",amount:2500,description:"FUT Champions coaching focused on a Rank 5 target. You keep full control of your account.",mode:"payment"},
...
app.post("/api/review-submit",auth,(req,res)=>{const{order_id,gameplay_url,notes}=req.body||{};const order=db.prepare("SELECT * FROM orders WHERE id=? AND user_id=? AND status='paid' AND service='review'").get(order_id,req.user.id);if(!order)return res.status(400).json({error:"You need a paid Gameplay Review order before submitting gameplay."});if(!gameplay_url||!/^https?:\/\//i.test(gameplay_url.trim()))return res.status(400).json({error:"Please enter a valid gameplay link starting with http:// or https://."});if(db.prepare("SELECT id FROM review_submissions WHERE order_id=?").get(order_id))return res.status(409).json({error:"Gameplay has already been submitted for this review."});db.prepare("INSERT INTO review_submissions(user_id,order_id,gameplay_url,notes,created_at) VALUES(?,?,?,?,?)").run(req.user.id,order_id,gameplay_url.trim(),(notes||"").trim(),now());res.json({ok:true})});
app.get("/api/dashboard",auth,(req,res)=>{const orders=db.prepare("SELECT o.*,b.id booking_id,s.starts_at FROM orders o LEFT JOIN bookings b ON b.order_id=o.id LEFT JOIN slots s ON s.id=b.slot_id WHERE o.user_id=? ORDER BY o.created_at DESC").all(req.user.id);const bookings=db.prepare("SELECT b.*,s.starts_at,s.duration_minutes,o.service FROM bookings b JOIN slots s ON s.id=b.slot_id JOIN orders o ON o.id=b.order_id WHERE b.user_id=? ORDER BY s.starts_at").all(req.user.id);const reviews=db.prepare("SELECT id,order_id,gameplay_url,notes,status,created_at FROM review_submissions WHERE user_id=? ORDER BY created_at DESC").all(req.user.id);res.json({user:publicUser(req.user),orders,bookings,reviews})});
